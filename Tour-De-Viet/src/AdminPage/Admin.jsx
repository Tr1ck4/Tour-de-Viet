...
export default function AdminPage() {
    ...
    const emptyTour = {
        townID: '', 
        tourName: '', 
        description: '{"Header": null, "Content": ""}', // Add description field with the initial value
        category: '', 
        price: '', 
        transportationID: ''
    };
    ...
    const handleSaveTour = async (updatedTour) => {
        try {
            await axios.put(`/api/tours/${updatedTour.tourName}`, updatedTour);
            setTours((prev) =>
                prev.map((tour) =>
    ...
    const handleSaveTransportation = async (updatedTransportation) => {
        try {
            await axios.put(`/api/transportations/${updatedTransportation.ID}`, updatedTransportation);
    ...
    const handleConfirmDelete = () => {
        ...
        if (type === 'pastTour') handleDeletePastTour(id);
        else if (type === 'ongoingTour') handleDeleteOngoingTour(id);
        else if (type === 'transportation') handleDeleteTransportation(id);
        ...
        return (
            <ul className="relative">
                ...
                <div className="h-96 no-scrollbar overflow-y-auto">
                    {onBook.map((booking) => (
                        <div className='relative' key={booking.tourName}>
                            ...
                            <button className='absolute top-5 right-20 w-1/12 h-14 bg-bright-yellow text-xl rounded-xl font-itim shadow-lg drop-shadow-2xl' onClick={() => confirmDelete('ongoingTour', booking.tourName)}>
                                Delete
                            </button>
                        </div>
                    ))}
                ...
                <div className="h-96 no-scrollbar overflow-y-auto">
                    {closeBook.map((booking) => (
                        <div className='relative' key={booking.tourName}>
                            ...
                            <button className='absolute top-5 right-20 w-1/12 h-14 bg-bright-yellow text-xl rounded-xl font-itim shadow-lg drop-shadow-2xl' onClick={() => confirmDelete('pastTour', booking.tourName)}>
                                Delete
                            </button>
                        </div>
                    ))}
            ...
            <ul className="relative">
                ...
                <div className="h-96 no-scrollbar overflow-y-auto">
                    {transportations.map((transportation) => (
                        <div className='relative' key={transportation.ID}>
                            ...
                            <button className='absolute top-5 right-20 w-1/12 h-14 bg-bright-yellow text-xl rounded-xl font-itim shadow-lg drop-shadow-2xl' onClick={() => confirmDelete('transportation', transportation.ID)}>
                                Delete
                            </button>
                        </div>
                    ))}
                    ...
                    <div className='relative overflow-hidden h-screen'>
                        ...
                        <div className='absolute w-4/5 h-4/5 rounded-xl inset-0 mx-auto mt-32 bg-dark-green bg-opacity-90 drop-shadow-2xl shadow-2xl font-itim'>
                            <div className='flex justify-between mx-12 my-5 '>
                                <button className='bg-bright-yellow w-1/4 h-14 text-xl rounded-xl font-itim shadow-lg drop-shadow-2xl' onClick={() => setCurrentPage('onGoing Tour')}>
                                    On Going
                                </button>
                                <button className='bg-bright-yellow w-1/4 h-14 ml-5 text-xl rounded-xl font-itim shadow-lg drop-shadow-2xl' onClick={() => setCurrentPage('past Tour')}>
                                    History
                                </button>
                                <button className='bg-bright-yellow w-1/4 h-14 text-xl ml-5 rounded-xl font-itim shadow-lg drop-shadow-2xl' onClick={() => setCurrentPage('transportation Page')}>
                                    Transportation
                                </button>
